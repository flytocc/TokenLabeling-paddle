...
| :------ | :--------- | :---- | :------- | :---------------- |
| TokenLabeling | lvvit_t | 分类 | 支持 | 多机多卡 <br> 混合精度 |
...
./test_tipc/
├── config                              # 配置文件目录
│   └── TokenLabeling                   # TokenLabeling系列模型测试配置文件目录
│       └── lvvit_t.txt                 # 基础训练预测配置文件
├── docs
│   ├── guide.png
│   ├── install.md                      # 配置运行环境
...
在`configs`目录下，**按模型系列划分为子目录**，子目录中存放所有该模型系列测试需要用到的配置文件，如`TokenLabeling`文件夹下存放了所有`TokenLabeling`系列模型的配置文件。配置文件的命名遵循如下规范：
