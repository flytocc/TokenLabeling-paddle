...
def one_hot(x, num_classes, on_value=1., off_value=0.):
    return F.one_hot(astype(x, paddle.int64), num_classes) * (on_value - off_value) + off_value
...
def get_label(label_maps, batch_coords, label_size=1):
    ...
    target_label = roi_align(
        label_maps, boxes, num_boxes, (label_size, label_size))
    if label_size > 1:
...
def get_labelmaps_with_coords(label_maps_topk, num_classes, on_value=1., off_value=0., label_size=1):
    '''
    Adapted from https://github.com/naver-ai/relabel_imagenet/blob/main/utils/relabel_functions.py
    ...
    ground_truth = astype(label_maps_topk[:, 2, 0, 0, 5], paddle.int64).flatten()
    ground_truth = one_hot(ground_truth, num_classes,
    ...
    if label_size == 1:
        return paddle.concat([ground_truth.reshape([B, C, 1]), token_label.reshape([B, C, 1])], axis=2)
    else:
        return paddle.concat([ground_truth.reshape([B, C, 1]), token_label], axis=2)
    ...
    if target.ndim > 2:
        if target.shape[1] == 3:
            y1 = get_labelmaps_with_coords(
                target, num_classes, on_value=on_value, off_value=off_value, label_size=label_size)
            y2 = y1.flip(0)
        else:
            raise ValueError("Not supported label type")
    else:
        y1 = one_hot(target, num_classes,
                     on_value=on_value, off_value=off_value)
        y2 = one_hot(target.flip(0), num_classes,
                     on_value=on_value, off_value=off_value)
